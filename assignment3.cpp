...
template <class T>
bool SmarterArray<T>::operator == (const SmarterArray<T> &L) const //return true if sizes are equal and elements at same indexes are ==
{
	if (L.size != this->size)
		return false;
	else
...
template<class K, class V>
int Map<K, V>::getSize() const { return A1.getSize(); }
template<class K, class V>
int Map<K, V>::findKey(const K& key) const { return A1.find(key); }
template<class K, class V>
int Map<K, V>::findValue(const V& value) const { return A2.find(value); }
template<class K, class V>
...
void Course::setCourseName(const string& n) {name = n;}
void Course::setCreditHours(const int& c) {creditHours = c;}
...
void Student::setFirstName(const string& firstName) {fn = firstName;}
void Student::setLastName(const string& lastName) {ln = lastName;}
...
{}
int SchoolManagementSystem::getNumberOfRegisteredStudents() const {return studentList.getSize();}
int SchoolManagementSystem::getNumberOfCoursesOffered() const {return courseList.getSize();}
...
{
	double total_credit_hours = 0.0, total_credit_points = 0.0;
	...
	for (int i = 0; i < studentMapList[studentIndex].getSize(); i++)
	{
		...
		{
			int course_index = studentMapList[studentIndex].getKeyAtIndex(i);
			double credit_hours = courseList[course_index].getCreditHours();
			total_credit_hours += credit_hours;
			
			if (studentMapList[studentIndex].getValueAtIndex(i) == 'A')
					total_credit_points += (4.0*credit_hours);
			else if (studentMapList[studentIndex].getValueAtIndex(i) == 'B')
					total_credit_points += (3.0*credit_hours);
			else if (studentMapList[studentIndex].getValueAtIndex(i) == 'C')
					total_credit_points += (2.0*credit_hours);
			else if (studentMapList[studentIndex].getValueAtIndex(i) == 'D')
					total_credit_points += (1.0*credit_hours);
...
{
	assert(this->studentMapList.getSize() != 0);
	int index;
	double gpa = 0.0;
	for (int i = 0; i < studentMapList.getSize(); i++)
		if (this->getStudentGPA(i) > gpa)
		{
			gpa = this->getStudentGPA(i);
			index = i;
		}
	return index;
}
bool SchoolManagementSystem::registerStudent(const Student &s)
{
	if (studentList.find(s) == -1)
	{
		...
		StudentMap newStudent;
		studentMapList.append(newStudent);
		return true;
	}
	else
		return false;
...
{
	assert((studentIndex >= 0 || studentIndex < studentList.getSize()) 
		&& (courseIndex >= 0 || courseIndex < courseList.getSize ()));
...
bool SchoolManagementSystem::assignLetterGrade(const int &studentIndex, const int &courseIndex, const char &letterGrade)
{
	...
	{
		int index = studentMapList[studentIndex].findKey(courseIndex);
		studentMapList[studentIndex].setValueAtIndex(index, letterGrade);
		return true;
	}
}
